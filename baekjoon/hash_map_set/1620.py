...
encyc = list(map(lambda i: [sys.stdin.readline()[:-1], i], range(1, N+1)))
e_dict = dict(encyc)
probs = list(map(lambda _: sys.stdin.readline()[:-1], range(M)))

for p in probs:
    if ord('1') <= ord(p[0]) <= ord('9'):
        # 숫자인 경우
        print(encyc[int(p)-1][0])
    else:
        print(e_dict[p])