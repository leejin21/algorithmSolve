...
반례: 동일한 정수가 삽입되는 경우
=> 식별자를 둬야 함
(https://neomindstd.github.io/문제풀이/boj7662/)

[게속되는 메모리 초과 해결 관련]
1. 우선 x = heapq.heappop()으로 뒀었는데 그걸 그냥 heaqp.heappop()으로 버리는 걸로 처리
2. 그래도 계속 메모리 초과가 떠서 commands = list(map()) 이렇게 I나 D에 관련한 명령들을 한꺼번에 받았던 걸
 -> c = readline().split()으로 for문에 나눠서 받음.
