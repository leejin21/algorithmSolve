...
def fullAttack(attacker, target):
    global destroyedTurretCnt
    if target.strength == 0:
        return

    target.strength -= attacker.strength
    if target.strength <= 0:
        destroyedTurretCnt += 1
        # print("dtc: ", destroyedTurretCnt)
        target.strength = 0
...
def lazerAttack(attacker, defender):
    ...
    while(turret.prevTurret):
        # print("r: ", turret.row, "c: ", turret.column)
        halfAttack(attacker, turret)
        turret = turret.prevTurret
        # print("next r: ", turret.row, "c: ", turret.column)
...
def findAlreadyDestroyedTurretCnt():
    n = 0
    for i in range(len(locationMap)):
        for j in range(len(locationMap[i])):
            turret = locationMap[i][j]
            if turret.strength == 0:
                n += 1
    return n
...
def solution():
    ...
    while(k<=K):
        ...
        if (destroyedTurretCnt == turretCnt - 1):
            break
...
turretCnt = N * M
destroyedTurretCnt += findAlreadyDestroyedTurretCnt()
solution()